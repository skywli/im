...
int main(int argc, char *argv[]) {
    ...
    if(access("../log/group",F_OK)==-1){
       mkdir("../log/group", 0766);
    }
    setenv("project_dir","../log/group/",1);
	
	ConfigFileReader::getInstance()->init(CONF_GROUP_URL);
	...
	GroupServer * server= GroupServer::getInstance();
	if (server->init() == -1) {
		LOGD("conn init fail");
		return -1;
	}
