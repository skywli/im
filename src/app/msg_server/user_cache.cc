	...
	user_t * UserCache::getUser(const std::string&  user_id)
	{
		...
		if (m_users.size() > 3000000) {
			...
			while (it != m_users.end()) {
				if (it->second->online==USER_STATE_LOGOUT) {
					user = it->second;
	...
	user_t * UserCache::queryUser(const std::string&  user_id)
	{
		...
		if (!user) {
			...
			if (res) {
				...
				char* pos = str;
				//state format:  nid:%d_fd:%d_dt:%d_v:%d_ph:%s_st:%d
				int i = 0;
				while (*pos) {
					if (*pos == ':') {
						pptr[i++] = pos + 1;
