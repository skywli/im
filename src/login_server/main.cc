...
int main(int argc, char *argv[]) {
    ...
    if(access("../log/login",F_OK)==-1){
       mkdir("../log/login", 0766);
    }
    setenv("project_dir","../log/login/",1);
	ConfigFileReader::getInstance()->init(CONF_LOGIN_URL);
	...
	LoginServer* login_server=LoginServer::getInstance();
	if (login_server->init() == -1) {
		LOGD("login_server init fail");
		return -1;
	}
